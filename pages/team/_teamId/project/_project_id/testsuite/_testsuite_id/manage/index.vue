  ...
  export default {
    ...
    computed: {
      user() {
        ...
        const user = this.$store.getters["modules/userModule/getUser"];
        user.writer = false;
        // userが該当するtestsuiteを所持する、teamの管理者である場合、user.writerをtrueにし、testcaseの登録/更新/削除、ソートモード移行を有効にする
        if (
          this.$store.state.modules.teamModule.teams.find(
        ...
        if (
          this.$store.state.modules.testsuiteModule.testsuits.find(
        ...
        if (
          this.$store.state.modules.testsuiteModule.testsuits
            .find(
              (element) => element.id == this.$nuxt.$route.params.testsuite_id
            )
      ...
      testCases() {
        ...
        return this.$store.state.modules.testcaseModule.testcases.filter(
          (element) =>
      ...
      testCaseSave(data) {
        ...
        if (data.id === 0)
          return this.$store.dispatch(
            "modules/testcaseModule/testcaseCreate",
            request
          );
        if (data.id != 0)
          return this.$store.dispatch(
            "modules/testcaseModule/testcaseUpdate",
            request
          );
      },
      testCaseDestroy(id) {
        ...
        return this.$store.dispatch(
          "modules/testcaseModule/testcaseDelete",
          request
        );
      },
      sortMode() {
