  ...
  export default {
    ...
    computed: {
      ...
      testCases() {
        ...
        this.$store.state.modules.demo.testcaseModule.testcases.filter(
          (element) =>
            element.testsuite_id == this.$nuxt.$route.params.testsuite_id
        );
        // ↑ から、ログインユーザーがtesterに指定されているtestcaseを取得する
        let testcases = this.$store.state.modules.demo.testcaseModule.testcases
          .filter(
            (element) =>
              element.testsuite_id == this.$nuxt.$route.params.testsuite_id
          )
    ...
    methods: {
      ...
      completion(data) {
        ...
        return this.$store.dispatch(
          "modules/demo/testcaseModule/testcaseConfirm",
          request
        );
      },
      profile(id) {
